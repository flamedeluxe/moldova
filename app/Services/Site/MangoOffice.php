...
Class MangoOffice {
    ...
    protected function putCmd($method, array $data, $command_id = null) {
        if (false !== $command_id) {
            if (is_null($command_id)) {
                $command_id = md5(json_encode($data) . $this->getSign($data));
            }
     ...
     * @param string $from_extension - внутренний номер сотрудника ВАТС
     * @param string $to_number - номер получателя
     * @param string $sms_sender - отправитель SMS (опционально)
     * @param string $command_id - уникальный ID команды (опционально)
     *
     * @return mixed
     */
    function sendSms($text, $from_extension, $to_number, $sms_sender = null, $command_id = null) {
        $data = [
            'text' => $text,
